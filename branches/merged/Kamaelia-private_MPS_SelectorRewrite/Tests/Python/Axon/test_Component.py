...
class Component_Test(unittest.TestCase):
   def test_SmokeTest_NoArguments(self):
      ...
      InboxNames = [ x for x in testcomponent.Inboxes ]
      OutboxNames = [ x for x in testcomponent.Outboxes ]
      self.failUnless(InboxNames.count("inbox")==1,"There should be one 'inbox' in the Inboxes structure.")
      self.failUnless(InboxNames.count("control")==1,"There should be one 'control' in the Inboxes structure.")
      self.failUnless(OutboxNames.count("outbox")==1,"There should be one 'outbox' in the Outboxes structure.")
      self.failUnless(OutboxNames.count("signal")==1,"There should be one 'signal' in the Outboxes structure.")
