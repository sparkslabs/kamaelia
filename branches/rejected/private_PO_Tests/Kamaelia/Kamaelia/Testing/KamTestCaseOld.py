...
# TODO: this might break something... why is background a singleton?
...
class KamTestCase(object):
    ...
    def initializeSystem(self, inputComponentUnderTest, outputComponentUnderTest = None):
        ...
        if hasattr(self, '_lf'):
            self._lf.shutdown()
        #self._lf = LikeFile.likefile(self._graph)
    ...
    def putYield(self, number = 1):
        self._messageAdder.addYield(number)
        
    def get(self, outbox):
        return self._messageStorer.getMessage(outbox)
        
    def assertOutboxEmpty(self, outbox, msg = None):
        if len(self._messageStorer.getMessages(outbox)) > 0:
    ...
    def _runSteps(self, steps):
        ...
        self._messageAdder.stopMessageAdder(steps)
        self._messageStorer.stopMessageStorer(steps)
    ...
    def assertNotStopping(self, msg = None, steps = None, clear = False):
        ...
        if len(threads) == 0:
            self.clearThreads()
            # then raise the failureException
            raise self.failureException(msg or "No process was still running")
        else:
            if clear:
                self.clearThreads()
