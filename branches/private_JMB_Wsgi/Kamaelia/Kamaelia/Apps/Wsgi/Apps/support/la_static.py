...
class StatusApp:
    ...
    def __call__(self, environ, start_response, headers=[('Content-type', 'text/html')]):
...
class Cling(object):
    ...
    not_found = StatusApp('404 Not Found', '404: File not found')
    ...
    def __call__(self, environ, start_response):
        ...
        if not (path.abspath(full_path) + '/')\
               .startswith(path.abspath(self.root) + '/'):
