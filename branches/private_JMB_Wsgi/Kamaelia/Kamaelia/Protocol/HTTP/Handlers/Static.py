...
def StaticFactory(indexfilename='index.html', homedirectory='htdocs', buffersize=0):
    def _getStatic(request):
        return Static(request, indexfilename, homedirectory, buffersize)
...
class Static(component):
    ...
    -- buffersize - The amount of bytes to buffer before sending it out to the HTTP handler.  If zero is
                        given, no output will be buffered.  If -1 is given all output will be buffered until
                        the end of the file.
                        
                        (-1 is not recommended for large files)
    """
    Inboxes = {
        "inbox"        : "UNUSED",
        "control"      : "UNUSED",
    ...
    def main(self):
        ...
        filename = sanitizePath(self.request["PATH_INFO"])
        ...
        while not done:
            ...
            if (len(buffer) > self.buffersize) and self.buffersize != -1:
...
__kamaelia_components__  = ( Static, )
