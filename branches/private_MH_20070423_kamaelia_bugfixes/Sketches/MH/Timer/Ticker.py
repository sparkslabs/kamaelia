...
class Ticker(Axon.Component.component):
   ...
   Inboxes = { "inbox"        : "Specify (new) filename",
               ...
               "_fromTimer"   : "Notification from the timer",
             }
   Outboxes = { "outbox" : "NOT USED",
                "signal" : "Shutdown signalling & sending requests to PygameDisplay service",
                "_timerRegister" : "For registering with the timer service",
                "_toTimer"  : "For requesting notification from the timer service",
              }
          ...
          for line in lines:
              ...
              if len(words) == 0:
                  if blankcount:
                      ...
                      blankcount = 1
              for word in words:
                  for _ in self.waitUntil(last + self.delay):
                      self.handleAlpha()                     
                      yield _
                  self.handleAlpha()
