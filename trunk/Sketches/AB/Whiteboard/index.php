...
if($_POST['delete']) {
    if (stristr($_POST['delete'],"/") === FALSE) {
        if (!unlink($_GET['dir'] . $_POST['delete'])) {
echo "fail";
}
    }
}

if ($_POST['fixnumbering']) {
    if ($_GET['dir'] == "Scribbles/") {
        $images = glob("Scribbles/slide.*.png");
        sort($images);
        for($i=0;$i<count($images);$i++) {
            $currentslidenum = str_replace(".png","",str_replace("slide.","",$images[$i]));
            if ($currentslidenum != ($i+1)) {
                //Do rename
                if (!rename($images[$i],"Scribbles/slide." . ($i+1) . ".png")) {
                    echo "Rename error for " . $images[$i];
                }
            }
        }
    }
...
if($allowuploads && $_FILES['file'] && ($leadon == "Decks/")) {
	$upload = true;
	if(!$overwrite) {
	...
	if($upload) {
        ...
        chmod($leadon . $_FILES['file']['name'],0666);
	}
}
...
<div id="container">
  ...
  <div id="listingcontainer">
	...
	?>
	<div><a href="<?=$fileurl;?>" class="<?=$class;?>"<?=$thumb2;?>><img src="dlf/<?=$icon;?>" alt="<?=$files[$i];?>" /><strong><?=$filename;?></strong> <em><?=round(filesize($leadon.$files[$i])/1024);?>KB</em> <?=date ("M d Y h:i:s A", filemtime($leadon.$files[$i]));?><?=$thumb;?></a><form method="post" style="display: inline" action="<?=$_SERVER['PHP_SELF'];?>?dir=<?=urlencode($leadon);?>" enctype="multipart/form-data"><input type="hidden" name="delete" value="<? echo $filename; ?>"><input type="submit" value="Delete" style="margin-bottom: 5px; margin-left: 5px"></form><? if ($leadon == "Scribbles/") {?><form method="post" style="display: inline" action="<?=$_SERVER['PHP_SELF'];?>?dir=<?=urlencode($leadon);?>" enctype="multipart/form-data"><input type="hidden" name="fixnumbering" value="fixnumbering"><input type="submit" value="Fix Numbering" style="margin-bottom: 5px; margin-left: 5px"></form><? } ?></div>
	<?
		if($class=='b') $class='w';
		else $class = 'b';	
	}	
