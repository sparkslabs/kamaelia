...
class InfiniteChooser_Internal_InitialisationTests(unittest.TestCase):
   def test_Instantiate_NoArgs(self):
      "__init__ - Creating empty InfiniteChooser is fine"
      x=InfiniteChooser()
   ...
   def test_Instantiate_ArgIterator(self):
      ...
      x=InfiniteChooser(xrange(0,5))
      self.assert_(iter_eq(x.items,xrange(1,5)), "__init__ right number of items")

      
      
class InfiniteChooser_Internal_IterateTests(unittest.TestCase):
