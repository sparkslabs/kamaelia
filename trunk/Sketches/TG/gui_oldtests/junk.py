...
class label(box):
    def __init__(self, text, width, height, x = 0, y = 0,
                        textcolour = colours["black"], bgcolour = colours["white"]):
        super(label, self).__init__(width, height, x = x, y = y, colour = bgcolour)
        
        self.text = text
        self.textcolour = textcolour
        self.font = pygame.font.SysFont("Times New Roman", 25)
        self.textim = self.font.render(text, True, textcolour, bgcolour).convert()
        self.xborder = (self.width - self.textim.get_width())/2
        self.yborder = (self.height - self.textim.get_height())/2

    def handleMouseDown(self, e):
        print 'clicked me!', self.text
        self.parent.floating = floating(self.x, self.y, self)

    def draw(self, surface):
        super(label, self).draw(surface)
        x = self.x + self.xborder
        y = self.y + self.yborder
        
        surface.blit(self.textim, (x, y))

class box:
    def __init__(self, screen, h, w, colour, xarc, yarc):
        self.screen = screen
        screensize = self.screen.get_size()
        self.screenwidth = screensize[0]
        self.screenheight = screensize[1]

        self.x = screensize[0]/2 - w/2
        self.y = screensize[1]/2 - h/2
        self.width = w
        self.height = h
        self.xarc = xarc #proportion of width to round
        self.yarc = yarc #proportion of height to round
        self.colour = colour
        self.setShapes()
    
    def setShapes(self):
        xarc = self.xarc*self.width #in pixels
        yarc = self.yarc*self.height #in pixels
        centreh = self.height - 2*yarc
        centrew = self.width - 2*xarc
        
        self.crect = Rect(self.x+xarc, self.y+yarc, centrew, centreh)
        self.lrect = Rect(self.x, self.y+yarc, xarc, centreh)
        self.rrect = Rect(self.x+xarc+centrew, self.y+yarc, xarc, centreh)
        self.trect = Rect(self.x+xarc, self.y, centrew, yarc)
        self.brect = Rect(self.x+xarc, self.y+yarc+centreh, centrew, yarc)
        
        self.rad = int(xarc if xarc < yarc else yarc)
        
        xarc *= 2
        yarc *= 2
        
        self.arc0rect = Rect(self.x, self.y, xarc, yarc)
        self.arc1rect = Rect(self.x+centrew, self.y, xarc, yarc)
        self.arc2rect = Rect(self.x, self.y+centreh, xarc, yarc)
        self.arc3rect = Rect(self.x+centrew, self.y+centreh, xarc, yarc)
    
    def draw(self):
        self.setShapes()
        
        rect(self.screen, self.colour, self.crect)
        rect(self.screen, self.colour, self.lrect)
        rect(self.screen, self.colour, self.rrect)
        rect(self.screen, self.colour, self.trect)
        rect(self.screen, self.colour, self.brect)
        
        ellipse(self.screen, self.colour, self.arc0rect, self.rad)
        ellipse(self.screen, self.colour, self.arc1rect, self.rad)
        ellipse(self.screen, self.colour, self.arc2rect, self.rad)
        ellipse(self.screen, self.colour, self.arc3rect, self.rad)