...
This component implements the basic functionality of an Interactor. An Interactor listens to events of another component and tranlates them into movement which is applied to the victim component. It provides methods to be overridden for adding functionality.
...
This interactor would rotate an object 90 degrees around the Y axis if the object is clicked. For examples of how to create more complex Interactors have a look at the files *Interactor.py.

Applying an interactor to a component is straightforward. The following example shows a MatchedInteractor and a RotationInteractor each interacting with a SimpleCube:

    CUBE1 = SimpleCube(size=(1,1,1), position=(1,0,-15)).activate()
    CUBE2 = SimpleCube(size=(1,1,1), position=(-1,0,-15)).activate()
...
The event identifier of the victim component gets saved in self.identifier. Use this variable in event handling to determine if the victim component has been hit.
...
if __name__=="__main__":
    ...
    from MatchedTranslationInteractor import MatchedTranslationInteractor
    from SimpleRotationInteractor import SimpleRotationInteractor
