...
class PygameComponent(Axon.Component.component):
   ...
   def __init__(self, **argd):
       super(PygameComponent,self).__init__()
   ...
   def removeHandler(self, eventtype, handler):
        ...
        if self.eventHandlers.has_key(eventtype):
            if handler in self.eventHandlers[eventtype]:
                self.eventHandlers[eventtype].remove(handler) # Latent bugs in application will cause an error here               # Latent bugs in application will cause an error here
