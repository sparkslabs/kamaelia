...
In handleEvents() the component reacts to mouse events and, if a key is assigned, to the keydown event. If a mouse click happens when the mouse pointer is over the Label it gets "grabbed", which is visualised by shrinking the Label by a little amount, until the Label is released again. Only if the mouse Label is released over the Label widget it gets activated. On activation the Label gets rotated by 360 degrees around the X axis.

In frame() the Label gets rotated when it has been activated.
...
class Label(OpenGLComponent):
    ...
    def __init__(self, **argd):
        ...
        self.backgroundColour = argd.get("bgcolour", (220,220,220))
        self.foregroundColour = argd.get("fgcolour", (0,0,0))
        self.sideColour = argd.get("sidecolour", (220,220,250))
